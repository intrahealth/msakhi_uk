...
public class FamilyDetails_Adapter extends BaseAdapter {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        int StatusId = Validate.returnIntegerValue(stat);
        if (StatusId == 3) {
            ...
            tvFMName.setBackgroundResource(R.drawable.grayesheet);
            tvFMRelation.setBackgroundResource(R.drawable.grayesheet);

            Gridimage.setBackgroundResource(R.drawable.grayesheet);
        } else {
            tvFMName.setBackgroundResource(R.drawable.whitesheet);
            tvFMRelation.setBackgroundResource(R.drawable.whitesheet);

            Gridimage.setBackgroundResource(R.drawable.whitesheet);
        }
        ...
        tvFMName.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                // TODO Auto-generated method stub
                if (global.getiGlobalRoleID() != 3) {
                    if ((HHFamilyMember.get(position).getFather().length() > 0 && HHFamilyMember.get(position).getFather() != null) || (HHFamilyMember.get(position).getMother()
                            .length() > 0 && HHFamilyMember.get(position).getMother() != null) || (HHFamilyMember.get(position).getSpouse().length() > 0 && HHFamilyMember.get(
                        ...
                        showdataspouse(sHHFamilyMemberGUID, sfather, sMother, sSpouse);

                    } else {
                        if (imarragestatus == 1) {
                            showRelationPopUp(igenderid, imarragestatus, sHHFamilyMemberGUID, HHFamilyMember.get(position).getHHSurveyGUID());
                        } else if (irelationid == 9 || irelationid == 10
                                || irelationid == 11 || irelationid == 12
                                || irelationid == 17 || irelationid == 18
                                || irelationid == 23 || irelationid == 24) {
                            showRelationChildrenPopUp(sHHFamilyMemberGUID, HHFamilyMember.get(position).getHHSurveyGUID());
                        }
                    }
        ...
        btnDelete.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (countdata(HHFamilyMember.get(position).getHHFamilyMemberGUID()) == 0) {
                    CustomAlert(HHFamilyMember.get(position).getHHFamilyMemberGUID());
                } else {
    ...
    public int countdata(String GUID) {
        int total = 0;
        try {
            String sqltblChild = "select count(*) from  from tblChild where ChildGUID='"
                    + GUID + "'";
            String sqltblPregnant_woman = "select count(*)  from tblPregnant_woman where HHFamilyMemberGUID='"
                    + GUID + "'";
            String sqltblncdcbac = "select count(*)  from tblncdcbac where HHFamilyMemberGUID='"
                    + GUID + "'";
            String sqltblncdfollowup = "select count(*)  from tblncdfollowup where HHFamilyMemberGUID='"
                    + GUID + "'";
            String sqltblncdscreening = "select count(*)  from tblncdscreening where HHFamilyMemberGUID='"
                    + GUID + "'";
            String sqltblFP_followup = "select count(*)  from tblFP_followup where WomenName_Guid='"
                    + GUID + "'";

            String sqlTbl_HHFamilyMember = "select count(*) from Tbl_HHFamilyMember where HHFamilyMemberGUID='"
                    + GUID + "' and IsEdited=0";
            total =  dataProvider.getMaxRecord(sqlTbl_HHFamilyMember) + dataProvider.getMaxRecord(sqltblChild) + dataProvider.getMaxRecord(sqltblPregnant_woman) + dataProvider.getMaxRecord(sqltblncdcbac) + dataProvider.getMaxRecord(sqltblncdfollowup) + dataProvider.getMaxRecord(sqltblncdscreening) + dataProvider.getMaxRecord(sqltblFP_followup);
            return total;
        } catch (Exception e) {
            e.printStackTrace();
            return total;
    ...
    public void deletedata(String GUID) {
        ...
        String sqlTbl_HHFamilyMember = "Delete from Tbl_HHFamilyMember where HHFamilyMemberGUID='"
                + GUID + "'";
        ...
        Intent in = new Intent(context, Intrahealth_Tab_Activity.class);
        ...
        btnDead.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (!RelationID.equalsIgnoreCase("null")) {
                    id = Validate.returnIntegerValue(RelationID);
                }
                CustomAlertSave(
        ...
        btnsave.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (sFatherGUID.length() > 0 || SmotherGUID.length() > 0) {
                    ...
                    int ireturnvalue = dataProvider.savefathermotherdata(showRelationChildrenPopUp, sFatherGUID, SmotherGUID);
                    if (ireturnvalue == 1) {
                        String query = "update Tbl_HHSurvey set IsEdited=1 where HHSurveyGUID='"
                                + hhguid + "'";
                        dataProvider.executeSql(query);
                        ...
                        String sqlmember = "";
                        sqlmember = "select * from  Tbl_HHFamilyMember where HHFamilyMemberGUID='"
                                + showRelationChildrenPopUp + "'";
                        data = dataProvider.getDynamicVal(sqlmember);
                        if (data != null) {
                            String sqlchild = "";
                            sqlchild = "select count(*) from  tblChild where HHFamilyMemberGUID='"
                                    + SmotherGUID + "' and Child_dob='" + data.get(0).get("DateOfBirth") + "'";
                            int childcount = dataProvider.getMaxRecord(sqlchild);
                            if (childcount == 0) {
                                String sqlinsertchild = "Insert into tblChild(AshaID,ANMID,HHGUID,HHFamilyMemberGUID,ChildGUID,MotherGUID,Child_dob,Gender,child_name,created_on,created_by,IsEdited)values('" + data.get(0).get("AshaID") + "','" + data.get(0).get("ANMID") + "','" + data.get(0).get("HHSurveyGUID") + "','" + data.get(0).get("Mother") + "','" + data.get(0).get("HHFamilyMemberGUID") + "','" + data.get(0).get("Mother") + "','" + data.get(0).get("DateOfBirth") + "','" + gender(Validate.returnIntegerValue(data.get(0).get("GenderID"))) + "','" + data.get(0).get("FamilyMemberName") + "','" + Validate.getcurrentdate() + "','" + global.getUserID() + "',1)";
                                dataProvider.executeSql(sqlinsertchild);
                            }
                        }
    ...
    public void showdataspouse(final String sHHFamilyMemberGUID,
        ...
        if (spouse.length() > 0) {
            ...
            int genderid = Validate.returnIntegerValue(gender);
            if (genderid == 1) {
