...
//		tvChild.setBackgroundColor(color.holo_orange_light);
        ...
        if (global.getiGlobalRoleID() == 4) {
            ...
            //((newbornGrid)context).fillgrid(1);
            String sVerify = "Select Verify from tblAFVerify where ModuleGUID='" + Child.get(position).getChildGUID() + "'";
            String verifyValue = dataProvider.getRecord(sVerify);

            if (verifyValue.length() > 0) {
                int iVerify = Integer.valueOf(verifyValue);
                if (iVerify == 1) {
                    tvChild.setBackgroundResource(R.drawable.aforangesheet);
                } else if (iVerify == 2) {
                    tvChild.setBackgroundResource(R.drawable.afgreensheet);
                } else if (iVerify == 3) {
                    tvChild.setBackgroundResource(R.drawable.afredsheet);
                }
            }
        } else {
