...
public class VHND_Perform_Adapter extends BaseAdapter {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			TableRow RadioButton_tblrow = (TableRow) gridview
					.findViewById(R.id.RadioButton_tblrow);
			if (Integer.valueOf(VHND_per.get(position).getID()) == 0) {
				Item.setGravity(Gravity.CENTER);
				RadioButton_tblrow.setVisibility(View.GONE);
				ID.setVisibility(View.GONE);

				// set for Table row
				ViewGroup.LayoutParams layoutpParams = new ViewGroup.LayoutParams(
						150, 100);
				RadioButton_tblrow.setLayoutParams(layoutpParams);
				Item.setTextColor(Color.BLACK);
				Typeface boldTypeface = Typeface
						.defaultFromStyle(Typeface.BOLD);
				Item.setTypeface(boldTypeface);
				Item.setTextSize(18);
			}
