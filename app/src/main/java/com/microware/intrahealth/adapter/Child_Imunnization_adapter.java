...
public class Child_Imunnization_adapter extends BaseAdapter {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if(global.getiGlobalRoleID()==4){
          ...
          //  ((ImunizationChildList)context).fillgrid(2);
            String sVerify = "Select Verify from tblAFVerify where ModuleGUID='" + ChildImmun.get(position).getChildGUID() + "'";
            String verifyValue = dataProvider.getRecord(sVerify);

            if (verifyValue.length() > 0) {
                int iVerify = Integer.valueOf(verifyValue);
                if (iVerify == 1) {
                    tvparentsname.setBackgroundResource(R.drawable.aforangesheet);
                } else if (iVerify == 2) {
                    tvparentsname.setBackgroundResource(R.drawable.afgreensheet);
                } else if (iVerify == 3) {
                    tvparentsname.setBackgroundResource(R.drawable.afredsheet);
                }
            }
        }else{
