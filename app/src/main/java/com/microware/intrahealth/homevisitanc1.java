...
public class homevisitanc1 extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// "page6"};
		adapter = new TouchImageAdapter(homevisitanc1.this);
	...
	class TouchImageAdapter extends PagerAdapter {
		...
		TblANCVisit AncVisitsave = new TblANCVisit();
		...
		public View instantiateItem(ViewGroup container, int position) {
			...
			final TableRow tblrdgrppn = (TableRow) itemView
					.findViewById(R.id.tblrdgrppn);
			RadioGroup rdgrp = (RadioGroup) itemView.findViewById(R.id.rdgrp);
			RadioGroup rdgrppn = (RadioGroup) itemView
					.findViewById(R.id.rdgrppn);
			...
			RadioButton radiopositive = (RadioButton) itemView
					.findViewById(R.id.radiopositive);
			RadioButton radionegative = (RadioButton) itemView
					.findViewById(R.id.radionegative);
			...
			rdgrp.setOnCheckedChangeListener(new OnCheckedChangeListener() {
				...
				public void onCheckedChanged(RadioGroup group, int checkedId) {
					...
					if (newpos == 8 || newpos == 9) {
						...
						// }else if(newpos==14||newpos==15){
						// if (checkedId == R.id.radioyes) {
						// tvdata.setText("1");
						// showhide(tblrdgrppn, 1);
						//
						// } else if (checkedId == R.id.radiono) {
						// showhide(tblrdgrppn, 0);
						// tvans.setText("");
						// tvdateans.setText("");
						// etAns.setText("");
						//
			...
			rdgrppn.setOnCheckedChangeListener(new OnCheckedChangeListener() {

				@Override
				public void onCheckedChanged(RadioGroup group, int checkedId) {
					// TODO Auto-generated method stub

					if (checkedId == R.id.radiopositive) {
						tvdatatv.setText("1");

					} else if (checkedId == R.id.radionegative) {
						tvdatatv.setText("0");
				...
				try {
					...
					if (value == 1) {
						...
						if (name2 != null && name2.length() > 0) {
							tvdateans.setText(name2);
							tvdatatv.setText(Validate.changeDateFormat(name2));
						} else {
							tvdateans.setText("");
							tvdatatv.setText("");
						}
			...
			if (position == 14) {
				try {
					...
					if (value == 1) {
						...
						// int ivalue=Validate.returnIntegerValue(name2);
						// if(ivalue==1){
						// showhide(tblrdgrppn, 1);
						// radiopositive.setChecked(true);
						// }else{
						// showhide(tblrdgrppn, 0);
						// radiopositive.setChecked(false);
						// }
						// }
						// showhide(tbletans, 1);
						// etAns.setText(name2);
	...
	public void onBackPressed() {
		...
		Intent in = new Intent(homevisitanc1.this, HomeVisit.class);
		finish();
		startActivity(in);
	}
	...
	public void savedata(int pos, String data, String datatv) {
		...
		if (pos == 0) {
			...
			if (datatv.length() > 0 && datatv != null) {
		...
		// }
			...
			int datanew = 0;
			if (datatv.length() > 0 && datatv != null) {
				datanew = Validate.returnIntegerValue(datatv);
			}
			sql = "update tblANCVisit set UrineTestsugar1YN=" + newdata
					+ ", UrineTestsugar1=" + datanew + " where VisitGUID='"
					+ global.getsGlobalANCVisitGUID() + "'and PWGUID='"
					+ global.getsGlobalPWGUID() + "'";
			System.out.println(sql);
			dataProvider.executeSql(sql);
			...
			sql = "update tblANCVisit set UrineTestAl1YN=" + newdata
					+ ", UrineTestAl1=" + datanew + " where VisitGUID='"
					+ global.getsGlobalANCVisitGUID() + "' and PWGUID='"
					+ global.getsGlobalPWGUID() + "'";
			System.out.println(sql);
			dataProvider.executeSql(sql);
