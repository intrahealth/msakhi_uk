...
import com.microware.intrahealth.adapter.Migrationselfadapter;
...
public class MigrationSelf extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.migrationself);
	...
	public void fillGrid() {
		HHFamilyMember = dataProvider.getHHFamilymemberData(
				...
				global.getGlobalHHFamilyMemberGUID(), 3);
		...
		if (HHFamilyMember != null) {
			...
			ImunizationGrid.setAdapter(new Migrationselfadapter(this,
					HHFamilyMember));
	...
	public int sCheckValidation() {
		...
		int iSizeANC = ImunizationGrid.getChildCount();
		int relation = 0;
		for (int i = 0; i < iSizeANC; i++) {
			try {
				if (HHFamilyMember.size() >= i) {
					ViewGroup gridChild = (ViewGroup) ImunizationGrid
							.getChildAt(i);

					Spinner spin_Relation = (Spinner) gridChild
							.findViewById(R.id.spin_Relation);
					if (spin_Relation.getSelectedItemPosition() == 1) {
						relation++;
						if (relation > 1) {
							return 6;
						}
					} else if (spin_Relation.getSelectedItemPosition() == 0) {
						return 7;
					}
	...
	public void showAlert(int iCheck) {
		if (iCheck == 2) {
			...
			CustomAlert(getResources().getString(R.string.pleaseselectrelation));
		}
	}
	...
	protected void onStart() {
		...
		if (networkInfoCheckConnection != null
				&& networkInfoCheckConnection.isConnected()
				&& networkInfoCheckConnection.isAvailable()) {
			GoogleAnalytics.getInstance(MigrationSelf.this)
	...
	protected void onStop() {
		...
		if (networkInfoCheckConnection != null
				&& networkInfoCheckConnection.isConnected()
				&& networkInfoCheckConnection.isAvailable()) {
			GoogleAnalytics.getInstance(MigrationSelf.this).reportActivityStop(
					this);
		}
	}
		...
		for (int i = 0; i < iSizeANC; i++) {
			try {
				if (HHFamilyMember.size() >= i) {
					...
					// gridChild.findViewById(R.id.Name);
					TextView tv_name = (TextView) gridChild
							.findViewById(R.id.tv_name);
					Spinner spin_Relation = (Spinner) gridChild
							.findViewById(R.id.spin_Relation);

					relation = returnid1(
							spin_Relation.getSelectedItemPosition() - 1, 6,
							global.getLanguage(), HHFamilyMember.get(i)
									.getGenderID());
