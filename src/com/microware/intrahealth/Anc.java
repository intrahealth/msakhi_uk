...
public class Anc extends Activity {
	...
	Spinner spinCheckup_place, spinBPYN, spinurineYN, spintt1, spinVDRLYN,
			...
			spincheckup, spindangersign, spinCalcium;
			...
			public void afterTextChanged(Editable s) {
				...
				if (s.length() > 0) {
					if (etweight.getText() != null
						...
						int height = Integer.valueOf(s.toString());
	...
	// public void showttdate(){
	// String visitno=String.valueOf(global.getVisitno()-1);
	// visit =
	// dataProvider.getTbl_VisitANCData(global.getsGlobalPWGUID(),visitno,4);
	// if(visit!=null && visit.size()>0){
	// if(visit.get(0).getTTfirstDoseDate().length()>0){
	// tvTT1date.setText(Validate.changeDateFormat(visit.get(0).getTTfirstDoseDate()));
	// tvTT1date.setEnabled(false);
	// }else{
	// tvTT1date.setText("");
	// }
	// if(visit.get(0).getTTsecondDoseDate().length()>0){
	// tvTT2date.setText(Validate.changeDateFormat(visit.get(0).getTTsecondDoseDate()));
	// tvTT2date.setEnabled(false);
	// }else{
	// tvTT2date.setText("");
	// }
	// if(visit.get(0).getTTBoosterDate().length()>0){
	// tvTT3date.setText(Validate.changeDateFormat(visit.get(0).getTTBoosterDate()));
	...
	public int sCheckValidation() {
		...
		if (actualheight > 180) {
			...
			return 2;
		} else if (etHeight.getText().toString() != null
				&& etHeight.getText().toString().length() < 3) {
			return 2;
		} else if (etweight.getText().toString() != null
				&& etweight.getText().toString().length() < 2) {
			return 3;
		} else if (etIFAnumber.getText().toString() != null
				&& etIFAnumber.getText().toString().length() > 0
				&& etIFAnumber.getText().toString().length() < 2) {
			return 4;
		} else if (etcalciumno.getText().toString() != null
				&& etcalciumno.getText().toString().length() > 0
				&& etcalciumno.getText().toString().length() < 2) {
			return 5;
		} else if (tvvisitdate.getText().toString().equalsIgnoreCase("null")
				&& tvvisitdate.getText().toString().length() == 0) {
			return 7;
		} else if (etHemoglobin.getText().toString() != null
				&& etHemoglobin.getText().toString().length() > 0) {
			int hemoglobin = Integer.valueOf(etHemoglobin.getText().toString());
			if (hemoglobin < 4) {
				return 6;
	...
	public void showwomandata() {
		...
		if (pregnant != null && pregnant.size() > 0) {
			...
			if (pregnant.get(0).getPWHeight() > 0) {
				etHeight.setText(String.valueOf(pregnant.get(0).getPWHeight()));
			} else {
				etHeight.setText("");
			}
			if (global.getVisitno() == 1) {
	...
	public void showdata() {
		...
		if (visit != null && visit.size() > 0) {
			...
			if (visit.get(0).getCheckupVisitDate() != null
				...
				dt = visit.get(0).getCheckupVisitDate();
				tvvisitdate.setText(Validate.changeDateFormat(dt).toString());
			} else {
				tvvisitdate.setText("");
			}
			...
			if (visit.get(0).getBirthWeight() > 0) {
				etweight.setText(String.valueOf(visit.get(0).getBirthWeight()));
			} else {
				etweight.setText("");
			}
			...
			if (visit.get(0).getBP() > 0) {
				spinBPYN.setSelection(returnPosition(7, visit.get(0).getBP()));
			} else {
			...
			if (visit.get(0).getDangerSign() > 0) {
				if (global.getVisitno() == 1) {
					spindangersign.setSelection(returnPosition(30, visit.get(0)
							.getDangerSign()));
				} else if (global.getVisitno() == 2) {
					spindangersign.setSelection(returnPosition(31, visit.get(0)
							.getDangerSign()));
				} else {
					spindangersign.setSelection(returnPosition(24, visit.get(0)
							.getDangerSign()));
				}
			} else {
			...
			if (visit.get(0).getUrineSugar() > 0) {
				spinurinesugar.setSelection(returnPosition(102, visit.get(0)
						.getUrineSugar()));
			} else {
				spinurinesugar.setSelection(0);
			}
			if (visit.get(0).getUrineAlbumin() > 0) {
				spinurineAlbumin.setSelection(returnPosition(102, visit.get(0)
						.getUrineAlbumin()));
			} else {
				spinurineAlbumin.setSelection(0);
			}
			...
			if (visit.get(0).getCheckupPlace() > 0) {
				spinCheckup_place.setSelection(returnPosition(101, visit.get(0)
						.getCheckupPlace()));
			} else {
				spinCheckup_place.setSelection(0);
			}
			if (visit.get(0).getUltraSound() > 0) {
				spinUltrasoundYN.setSelection(returnPosition(7, visit.get(0)
						.getUltraSound()));
			} else {
			...
			if (visit.get(0).getUltrasoundResult() > 0) {
				spinultrasoundresult.setSelection(returnPosition(13,
						visit.get(0).getUltrasoundResult()));
			} else {
				spinultrasoundresult.setSelection(0);
			}
	...
	public void sSavedata() {
		...
		if (global.getVisitno() == 1) {
			String sql = "Update  tblPregnant_woman set LMPDate='" + LMPDATE
					...
					+ global.getHishRisk() + " where PWGUID='"
					+ global.getsGlobalPWGUID() + "'";
		...
		if (global.getVisitno() < 4) {
			String sql1 = "update tblANCVisit set TT1TT2last2yr="
					...
					+ TTbooster + "'  where PWGUID='"
					+ global.getsGlobalPWGUID() + "' and Visit_No=" + visitno
					+ "";
