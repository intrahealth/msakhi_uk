...
public class FamilyDetails_Adapter extends BaseAdapter {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (HHFamilyMember.get(position).getMaritialStatusID() == 1
				&& HHFamilyMember.get(position).getGenderID() == 2) {
			...
			public void onClick(View v) {
				...
				if (sSpouseGUID.length() > 0) {
					...
					AlertDialog alertDialog = new AlertDialog.Builder(context)
							.create();
					alertDialog.setTitle("mSakhi");
					alertDialog.setMessage("Select Spouse name.");
					alertDialog.setButton("OK",
							new DialogInterface.OnClickListener() {
								@Override
								public void onClick(DialogInterface dialog,
				...
				if (StatusID == 3) {
					...
					CustomAlertSave(
							context.getResources()
									.getString(R.string.migratein), 1);
				} else {
					global.setMigrationGUID("");
					CustomAlertSave(
							context.getResources().getString(
									R.string.migrateout), 2);
				}
	...
	public void CustomAlertSave(String msg, final int id) {
	...
	public void showRelationChildrenPopUp(final String showRelationChildrenPopUp) {
		PicVideo_PreviewPopUp = new Dialog(context);
		Window window = PicVideo_PreviewPopUp.getWindow();
		PicVideo_PreviewPopUp.getWindow().setLayout(600, 400);
		// PicVideo_PreviewPopUp.getWindow().setBackgroundDrawable(
			...
			public void onClick(View v) {
				...
				if (sFatherGUID.length() > 0 && SmotherGUID.length() > 0) {
					...
					AlertDialog alertDialog = new AlertDialog.Builder(context)
							.create();
					alertDialog.setTitle("mSakhi");
					alertDialog.setMessage("Select Mother's/ father's.");
					alertDialog.setButton("OK",
							new DialogInterface.OnClickListener() {
								@Override
								public void onClick(DialogInterface dialog,
	...
	public void showdataspouse(final String sHHFamilyMemberGUID,
		...
		btnSave.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				// String sql =
				// "select Spouse from Tbl_HHFamilyMember where HHFamilyMemberGUID='"
				// + sFatherGUID + "'";
				// String mother = dataProvider.getRecord(sql);
				// ireturnvalue = dataProvider.savefathermotherdata(
				// sHHFamilyMemberGUID, sFatherGUID, mother);
				// if (ireturnvalue == 1) {
				//
				// AlertDialog alertDialog = new AlertDialog.Builder(
				// context).create();
				...
				//
				// }
				// } else if (SmotherGUID.length() > 0) {
				// // String sql =
				// //
				// "select Spouse from Tbl_HHFamilyMember where HHFamilyMemberGUID='"
				// // + SmotherGUID + "'";
				// // String Father = dataProvider.getRecord(sql);
				// ireturnvalue = dataProvider.savefathermotherdata(
				// sHHFamilyMemberGUID, "", SmotherGUID);
				// if (ireturnvalue == 1) {
				//
				// AlertDialog alertDialog = new AlertDialog.Builder(
				// context).create();
