...
import com.microware.intrahealth.ListViewActivity;
...
public class ListViewVistAncAdapter extends BaseAdapter {
...
ArrayList<Tbl_HHFamilyMember> spouse;
	public ListViewVistAncAdapter(Context context,
			ArrayList<TblANCVisit> VisitANC) {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			gridview = layoutInflater.inflate(R.layout.listview_gridadapter, null);
		} else {
		...
		pregnant=dataProvider.getPregnantWomendata(VisitANC.get(position).getPWGUID(), 1,ashaid);
//		TableRow tablrowvisit=(TableRow) gridview.findViewById(R.id.tablrowvisit);
...
//			imagedelete.setEnabled(false);
		...
		tvsno.setText(" check-up" + "-" + String.valueOf(VisitANC.get(position).getVisit_No()));
		tvlastvisit.setText(Validate.changeDateFormat(VisitANC.get(position).getCheckupVisitDate()));
		
		if(pregnant!=null && pregnant.size()>0){
		if(pregnant.get(0).getPWRegistrationDate()!=null && pregnant.get(0).getPWRegistrationDate().length()>0){
...
//					imageedit.setEnabled(true);
				} else 	if(diffDays>98 && VisitANC.get(position).getCheckupVisitDate()==null || VisitANC.get(position).getCheckupVisitDate().length()==0){
					tvsno.setBackgroundColor(context.getResources().getColor(R.color.Orange));
					tvlastvisit.setBackgroundColor(context.getResources().getColor(R.color.Orange));
//					imageedit.setEnabled(false);
				}
				if(VisitANC.get(position).getCheckupVisitDate()!=null && VisitANC.get(position).getCheckupVisitDate().length()>0 ){
...
//					imageedit.setEnabled(true);
					...
					if(diffDays >=91 && diffDays<=196) {
						...
						tvlastvisit.setBackgroundColor(Color.BLUE);
						imageedit.setEnabled(true);
				}else 	if(diffDays>196 &&(VisitANC.get(position).getCheckupVisitDate()==null || VisitANC.get(position).getCheckupVisitDate().length()==0)){
					tvsno.setBackgroundColor(context.getResources().getColor(R.color.Orange));
					tvlastvisit.setBackgroundColor(context.getResources().getColor(R.color.Orange));
//					imageedit.setEnabled(false);
				}
				...
				if(diffDays >=189 && diffDays<=252){
					...
					tvlastvisit.setBackgroundColor(Color.BLUE);
					imageedit.setEnabled(true);
				}else if(diffDays>252 &&( VisitANC.get(position).getCheckupVisitDate()==null || VisitANC.get(position).getCheckupVisitDate().length()==0)){
					tvsno.setBackgroundColor(context.getResources().getColor(R.color.Orange));
					tvlastvisit.setBackgroundColor(context.getResources().getColor(R.color.Orange));
//					imageedit.setEnabled(false);
				}
				...
				if(diffDays>=245 && ( VisitANC.get(position).getCheckupVisitDate()==null || VisitANC.get(position).getCheckupVisitDate().length()==0)){
					tvsno.setBackgroundColor(Color.BLUE);
					tvlastvisit.setBackgroundColor(Color.BLUE);
					imageedit.setEnabled(true);
...
//				global.setiCurrentActiveTab(0);
				spouse=dataProvider.getAllmalename(pregnant.get(0).getHHFamilyMemberGUID());
				...
				((ListViewActivity) context).finish();
