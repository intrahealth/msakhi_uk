...
public class AncQuestionAdapter extends BaseAdapter {

	ArrayList<tbl_pregnantwomen> pregnant;
	Context context;
	Global global;
	DataProvider dataProvider;
	ArrayList<tblmstANCQues> Questions;
	int iflag = 0;
	int j = 0;
	Dialog timepic;
	TimePicker time;
	...
	public AncQuestionAdapter(Context context,
			ArrayList<tblmstANCQues> Questions, int currentpage, int noofcount) {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			gridview = layoutInflater.inflate(
					R.layout.ancquestions_gridadapter, null);
		} else {
		...
		TextView visitdate = (TextView) gridview.findViewById(R.id.visitdate);
		...
		if (Questions.get(CurrentPage).getAnsField() != null
				...
				&& !Questions.get(CurrentPage).getAnsField()
		...
		// if(CurrentPage==1){
		//
		// if(global.getDateMonth()!=null && global.getDateMonth().length()>0){
		// newage=global.getDateMonth().split("/");
		// tvDOB.setText(String.valueOf(newage[0]+"months"+newage[1]+"days"));
		...
		if ((global.getsGlobalANCVisitGUID() != null && global
				.getsGlobalANCVisitGUID().length() > 0)
				&& (global.getsGlobalPWGUID() != null && global
						.getsGlobalPWGUID().length() > 0)) {

			showdata(Questions.get(CurrentPage).getAnsField(), ans, rdgrp,
					tvdate, ansQ, tblrdgrp2, tblrdgrp3, etans, visitdate,
					tblrdgrptime, tvtime);
		} else {
			//
		...
		if (Questions.get(CurrentPage).getAudio_filename() != null
			...
			if (noofcount > 0) {
				if (Questions.get(CurrentPage).getAudio_filename()
						.contains("$")) {

					final String[] ar = Questions.get(CurrentPage)
							.getAudio_filename().split("[$]");
					// int returnvalue1=0;
					for (int i = 0; i < ar.length; i++) {
						((AncQuestionActivity) context).PlayAudio1(ar,
								returnvalue);
						returnvalue = 1;
					}
				} else {
					((AncQuestionActivity) context).PlayAudio(Questions.get(
	...
	public void showdata(String Field, String ans, RadioGroup rdgrp,
			...
			TableRow tblrdgrp3, EditText etans, TextView visitdate,
			TableRow tblrdgrptime, TextView tvtime) {
		String sqldate = "Select HomeVisitDate from tblANCVisit where VisitGUID='"
				+ global.getsGlobalANCVisitGUID()
				+ "' and PWGUID='"
				+ global.getsGlobalPWGUID() + "'";
		String Date = dataProvider.getRecord(sqldate);
		if (Date != null && Date.length() > 0) {
			visitdate.setText(String.valueOf(Date));
		} else {
			visitdate.setText(Validate.getcurrentdate());
		}
		int newvalue = 0;
		if (ans != null && ans.length() > 0) {
			if (ans.contains("$")) {
				...
				String sql = "select " + Field
						+ " from tblANCVisit where VisitGUID='"
